#ifndef FRENETROS_OBST_HPP_
#define FRENETROS_OBST_HPP_
...
void find_nearest_in_global_path(vecD &global_x, vecD &global_y, double &min_x, double &min_y,
								 double &min_dis, int &min_id, int flag, double &pose_x, double &pose_y, FrenetPath &path)
{
	double bot_x, bot_y;
...
int initial_conditions_new(Spline2D &csp, vecD &global_s, vecD &global_x, vecD &global_y, vecD &global_R, vecD &global_yaw, double &s0, double &c_speed, double &c_d, double &c_d_d,double &c_d_dd, double &bot_yaw, double &pose_x, double &pose_y, FrenetPath &path, double v_speed)
{
	//차량의 선속도 계산식
	// double vx = odom.twist.twist.linear.x;
	// double vy = odom.twist.twist.linear.y;
	// double v = sqrt(vx * vx + vy * vy);
	double v = v_speed; // 속도값 m/s
	double min_x, min_y;
	int min_id;

	// getting d
	find_nearest_in_global_path(global_x, global_y, min_x, min_y, c_d, min_id, 0, pose_x, pose_y, path);

	// deciding the sign for d
	pair<double, double> vec1, vec2;
	vec1.first = pose_x - global_x[min_id];
	vec1.second = pose_y - global_y[min_id];
...
// 		// cerr << setprecision(3);
// 		// cerr << "Frequency : " << frequency << " Hz" << endl;
		
// 		ros::spinOnce();
// 	}
// }

#endif // FRENETROS_OBST_HPP_
